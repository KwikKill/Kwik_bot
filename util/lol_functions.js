    ...
    async start_services(route, debug = false) {
        ...
        while (this.services[route]["queue"]["updates"].length > 0) {
            ...
            if (current["type"] === "match") {
                ...
                const update = this.basicUpdate(route, current, timer1, debug);
                this.services[route]["promises"].push(update);
            }
        }
        logger.warn("lol (update)[" + route + "] : Reached steady state, " + this.services[route]["promises"].length + " promises pending");

        // Wait for all promises to finish
        await Promise.all(this.services[route]["promises"]);
        const end = new Date();
        if (debug) {
