				...
				for(const j in di) {
					for(const i in di[j]) {
						for(const h in di[j][i]) {
							...
							context.font = '12px sans-serif';
							context.fillStyle = '#000000';
							context.fillText(description3, (Math.floor(47 + 297 * (start.getUTCDay() - 1) + width*h) + Math.floor(47 + 297 * (start.getUTCDay() - 1) + width*h) + width)/2, Math.floor(40 + 45.2 * (start.getHours() - 7 + (start.getMinutes()/60))) + 15 + Math.floor(8*(hours - 1 + (minutes/60))), width);
